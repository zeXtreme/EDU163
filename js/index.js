...
function page(opt){
    ...
    var prvPage = document.querySelector('.m-pager .prv');
    var pageNum = document.querySelectorAll('.m-pager .pg');
    var nxtPage = document.querySelector('.m-pager .nxt');
    
    //当前页不等于1时上一页可选
    if(nowNum != 1){
        prvPage.setAttribute('index',nowNum - 1);
        removeClass(prvPage,'f-dis');
    }
    else{
        addClass(prvPage,'f-dis');
    }
    //生成具体页数，总页数小于等于可显示页数的情况
    if(allNum <= childLength){
        for(var i=1; i <= allNum; i++){            
            pageNum[i-1].setAttribute('index',i);
            pageNum[i-1].innerText = i;
        }
    }
    //生成具体页数，总页数大于可显示页数的情况
        ...
        for(var i=1; i <= childLength; i++){
            ...
            //当前页是倒数第1或倒数第2
                ...
                pageNum[i-1].setAttribute('index',allNum - childLength +i);
                pageNum[i-1].innerText = allNum - childLength +i;
            }
            //当前页在可显示页数一半+1的位置显示，例如可以显示8页，当前页就在第5个位置
            else{
                pageNum[i-1].setAttribute('index',nowNum - point + i);
                pageNum[i-1].innerText = nowNum - point + i;
            }
        }
    }
    //当前页不是最后一页时显示下一页
    if(allNum != nowNum){
        nxtPage.setAttribute('index',nowNum + 1);
        removeClass(nxtPage,'f-dis');
    }
    else{
        addClass(nxtPage,'f-dis');        
    }
